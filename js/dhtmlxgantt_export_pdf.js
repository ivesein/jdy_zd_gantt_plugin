;(function() {
	...
	function add_export_methods(gantt) {
		...
		gantt._send_to_export = function(data, type) {
			if (data.config) mark_columns(data, type)
		...
		gantt._create_hidden_form = function() {
			if (!this._hidden_export_form) {
				var t = (this._hidden_export_form = document.createElement(
					"div"
				))
		...
		function copy_object_plain(obj) {
			var text = gantt.templates.task_text(
				obj.start_date,
				obj.end_date,
				obj
			)
		...
		function fix_columns(gantt, columns) {
			for (var i = 0; i < columns.length; i++) {
				columns[i].label =
					columns[i].label ||
					gantt.locale.labels["column_" + columns[i].name]
			...
			if (gantt._scale_helpers) {
				...
				if (timeline) {
					...
					if (
						...
						availWidth = Math.max(
							gantt.config.autosize_min_width,
							0
						)
					}
...
;(function() {
	...
	function on_circ_dependency_find(link, links, usedTasks, skippedLinks) {
		if (link) {
			if (
				gantt.callEvent("onExportCircularDependency", [link.id, link])
			) {
	...
	function exportConfig(data, config) {
		...
		var manual =
			config.auto_scheduling === undefined
				? false
